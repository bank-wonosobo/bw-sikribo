            ...
            <div class="card-body">
                ...
                <table class="table datatable">
                    ...
                    <tr>
                        ...
                        <td>
                            <span class="badge @if ($permohonan->status == 'SELESAI') bg-success @elseif($permohonan->status == 'PROSES SLIK') bg-primary @elseif($permohonan->status == 'TOLAK') bg-danger @else bg-light text-dark @endif">{{ ucwords($permohonan->status) }}</span>
                        </td>
                        <td>
                            @if ($permohonan->sliks->count() == 0)
                                <a class="btn btn-sm btn-warning text-white" href="{{ route('admin.slik.create', ['permohonan_slik_id' => $permohonan->id]) }}">Input Debitur</a>
                            @elseif ($slik->status == 'TOLAK')
